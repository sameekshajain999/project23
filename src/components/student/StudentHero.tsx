      ...
      <div className="px-5 pb-4 pt-0 relative">
        <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between gap-4 -mt-10 sm:-mt-12 mb-3">
          ...
          <div className="flex flex-wrap sm:flex-nowrap items-center gap-2.5 w-full lg:w-auto">
            ...
            {/* My Applications Quick Action */}
            {onOpenMyApplications && (
              <button
                id="my-applications-hero-btn"
                onClick={onOpenMyApplications}
                className="flex-1 sm:flex-none flex items-center justify-center space-x-1.5 px-3.5 py-2.5 bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-300 rounded-xl text-xs font-bold transition-all active:scale-95 cursor-pointer shadow-2xs"
                title="View your submitted applications & status"
              >
                <Briefcase className="w-3.5 h-3.5 text-amber-700" />
                <span>My Applications</span>
                <span className="ml-1 px-1.5 py-0.2 rounded-full bg-amber-200 text-amber-950 text-[10px] font-mono font-bold">
                  {applicationsCount ?? 0}
                </span>
              </button>
            )}
        ...
        {/* Verified Badges: Compact Single-Line Tag/Badge List */}
        <div className="mt-3 pt-2.5 border-t border-slate-100 flex flex-wrap sm:flex-nowrap items-center gap-2 text-xs">
          <div className="flex items-center space-x-1.5 text-slate-700 font-bold shrink-0">
            <Award className="w-3.5 h-3.5 text-amber-600" />
            <span className="text-[11px]">Tamper-Proof Badges:</span>
          </div>
          ...
          <div className="flex items-center flex-wrap gap-1.5 overflow-x-auto py-0.5">
            {student.badges.map((badge) => (
              <button
                key={badge.id}
                id={`badge-tag-${badge.id}`}
                onClick={() => setSelectedBadge(badge)}
                className="inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-full bg-slate-50 hover:bg-emerald-50 border border-slate-200 hover:border-emerald-300 text-slate-700 hover:text-emerald-900 transition-all text-[11px] font-medium cursor-pointer shrink-0 shadow-2xs"
                title={`Click to view cryptographic verification for ${badge.title} (${badge.issuer})`}
              >
